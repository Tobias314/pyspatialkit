...
################################## Conda environment ##################################
RUN mamba create -n env python=3.8 -y
...
################################## Conda Dependencies ##################################
# Conda
RUN conda activate env; pip install numpy matplotlib shapely pyproj rasterio geopandas open3d trimesh mapbox_earcut pylas triangle
RUN conda activate env; pip install python-fcl cython pygeos scikit-image
...
# RUN mamba install -n env -c conda-forge fcl -y
