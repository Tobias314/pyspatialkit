...
class TestGeoRasterLayer(unittest.TestCase):
    ...
    def test_read_write(self):
        ...
        self.raster_layer.write_data(raster)
        #raster = GeoRaster(GeoRect((9500,9500), (10500,10500), crs=NoneCRS()), np.ones((1000,1000)))
        #self.raster_layer.writer_raster_data(raster)
        res = self.raster_layer.get_data(GeoRect((0,0), (1000,1000), crs=NoneCRS()))
        self.assertEqual(res.data.sum(), 900*250)
        res = self.raster_layer.get_data(GeoRect((9500,9500), (10500,10500), crs=NoneCRS()))
