...
class TileDbSparseBackend:
    ...
    def __init__(self, bounds: Tuple[float, float, float, float, float, float], directory_path: Path, data_scheme: Dict[str, np.dtype] = {},
                 space_tile_size: Tuple[float, float] = (1, 1), base_density: int= 1000, data_tile_capacity=100,
                 fill_value = 0, build_pyramid: bool = True, num_pyramid_layers: Union[int, str] = 'all') -> None:
        self.bounds = np.array(bounds)
        self.size = self.bounds[3:] - self.bounds[:3]
        self.space_tile_size = np.array(self.space_tile_size)
