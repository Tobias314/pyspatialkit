...
class TileDbSparseBackend:
    ...
    def __init__(self, bounds: Tuple[float, float, float, float, float, float], directory_path: Path, data_scheme: Dict[str, np.dtype] = {},
        ...
        if not (AXIS_NAMES[0] in data_scheme and AXIS_NAMES[1] in data_scheme and AXIS_NAMES[2] in data_scheme):
            raise ValueError("At least {},{},{} must be in the data scheme!".format(AXIS_NAMES[0], AXIS_NAMES[1], AXIS_NAMES[2]))
        ...
        for i, layer in enumerate(self.levels):
            ...
            if not Path(path).exists():
                ...
                schema = tiledb.ArraySchema(domain=dom, sparse=True, capacity=self.data_tile_capacity, attrs=self.array_attributes.values())
                schema.check()
    ...
    def write_data(self, data: pd.DataFrame) -> None:
        ...
        if not (AXIS_NAMES[0] in columns and AXIS_NAMES[1] in columns and AXIS_NAMES[2] in columns):
            raise ValueError("data needs {},{},{} columns".format(AXIS_NAMES[0], AXIS_NAMES[1], AXIS_NAMES[2]))
        attributes = {}
        for column in columns:
            if column not in AXIS_NAMES_SET:
                if column not in self.array_attributes:
                    raise ValueError('Attribute {} does not exist!'.format(column))
        ...
        for level in range(1, self.num_pyramid_layers+1):
            ...
            with tiledb.SparseArray(write_db_path, mode='w') as db:
                for bounds in dirty_regions:
                    df = self.levels[level-1][1].df[bounds[0]:bounds[3], bounds[1]:bounds[4], bounds[2]:bounds[5]]
                    area_estimate = self._get_bounds_half_surface_area(bounds)
                    ...
                    num_points_target = int(np.clip(len(df) / POINT_PYRAMID_REDUCTION_FACTOR, points_lower_estimate, points_upper_bound))
                    if num_points_target < len(df):
                        df = df.sample(n=num_points_target)
                    attributes = {}
                    for key in df.columns:
                        if key not in AXIS_NAMES_SET:
                            attributes[key] = df[key].to_numpy() #TODO: to_numpy() might not be needed
                    db[df[AXIS_NAMES[0]], df[AXIS_NAMES[1]], df[AXIS_NAMES[2]]] = attributes
    ...
    def get_data(self, bounds: Tuple[float, float, float, float, float, float], attributes: Optional[Tuple[str]]=None, min_num_points: Optional[int] = None) -> pd.DataFrame:
        ...
        level = np.clip(1 + math.floor(math.log(density_estimate / self.base_point_density, 2)), 0, self.num_pyramid_layers)
        est_size = self.levels[level][1][bounds[0]:bounds[3], bounds[1]:bounds[4], bounds[2]:bounds[5]].est_result_size()
        if est_size < min_num_points:
            level+=1
            while level+1 <= self.num_pyramid_layers and self.levels[level][1][bounds[0]:bounds[3], bounds[1]:bounds[4], bounds[2]:bounds[5]].est_result_size() < min_num_points:
                level+=1
        elif est_size > min_num_points:
            while level-1 >= 0 and self.levels[level+1][1][bounds[0]:bounds[3], bounds[1]:bounds[4], bounds[2]:bounds[5]].est_result_size() > min_num_points:
                level-=1
    ...
    def get_data_for_level(self, bounds: Tuple[float, float, float, float, float ,float], level:int, attributes: Optional[Tuple[str]]=None):
        return self.levels[level][1].query(attrs=attributes, dims=AXIS_NAMES).df[bounds[0]:bounds[3], bounds[1]:bounds[4], bounds[2]:bounds[5]]
