...
def interpolate_holes(img: np.ndarray, mask) -> np.ndarray:
    t, m = interpolate_along_axis(img, mask)
    t, m = interpolate_along_axis(t, m)
    return t

def interpolate_along_axis(img: np.ndarray, mask: np.ndarray)-> Tuple[np.ndarray, np.ndarray]:
