...
class PySpatialKitServer(uvicorn.Server):
...
def build_fastapi_app(geostorage: GeoStorage) -> FastAPI:
    app = FastAPI()
...
def start_server(geostorage: GeoStorage, port=8080) -> None:
    ...
    app = build_fastapi_app(geostorage=geostorage)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(prog="PyspatialkitServer")
    parser.add_argument('geostorage', type=str)
    args = parser.parse_args()
    start_server(geostorage=GeoStorage(args.geostorage))